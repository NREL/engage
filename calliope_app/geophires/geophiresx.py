...
class Geophires(object):
    ...
    def run(self, input_params, output_file):
        ...
        output_params = dict(
            max_electricity_cap                     = 1,                       ### [MW] Conversion (Surface)                   ->     Maximum production capacity
            surface_plant_efficiency                = 1,                ### [%] Conversion (Surface)                    ->     Conversion Efficiency (instead of c-rate)
            surface_cost_to_electric_slope          = 1,                              ### [$/MW] Conversion (Surface)                   ->     Cost of Production Capacity
            surface_om_cost_to_electric_slope       = 1,                              ### [$/MW] Conversion (Surface)                   ->     Annual Fixed O&M Cost

            max_heat_cap                            = 1,                       ### [MW] Supply     (Subsurface)                ->     Maximum production capacity
            subsurface_cost_to_thermal_slope        = 1,                              ### [$/MW] Supply     (Subsurface)                ->     Cost of Production Capacity
            subsurface_om_cost_to_thermal_slope     = 1,                              ### [$/MW] Supply     (Subsurface)                ->     Annual Fixed O&M Cost
            interest_rate                           = 1*100,                        ### [%] maps to both conversion and supply      ->     Interest rate
            lifetime                                = 1                        ### [years] maps to both conversion and supply  ->     Lifetime and amortization period
        )
    ...
    def generate_config_files(self, input_params):
        ...
        Returns
        -------
            A list of io.StringIO file-like config
        """
        config_files = []
                ...
                for inum in injection_wells:
                    ...
                    for temp in temperatures:
                        ...
                        for line in self.template_data:
                            if line.startswith("Reservoir Depth,"):
                                line = line.replace(
                                    "Reservoir Depth,",
                                    "Reservoir Depth," + str(depth)
                                )
                            elif line.startswith("Number of Production Wells,"):
                                line = line.replace(
                                    "Number of Production Wells,",
                                    "Number of Production Wells," + str(pnum)
                                )
                            elif line.startswith("Number of Injection Wells,"):
                                line = line.replace(
                                    "Number of Injection Wells,",
                                    "Number of Injection Wells," + str(inum)
                                )
                            elif line.startswith("Maximum Temperature,"):
                                line = line.replace(
                                    "Maximum Temperature,",
                                    "Maximum Temperature," + str(temp)
                                )
                            elif line.startswith("Reservoir Density"):
                                line = line.replace(
                                    "Reservoir Density,",
                                    "Reservoir Density," + str(input_params.reservoir_density)
                                )
                            elif line.startswith("Reservoir Thermal Conductivity,"):
                                line = line.replace(
                                    "Reservoir Thermal Conductivity",
                                    "Reservoir Thermal Conductivity" + str(input_params.reservoir_thermal_conductivity)
                                )
                            elif line.startswith("Reservoir Heat Capacity,"):
                                line = line.replace(
                                    "Reservoir Heat Capacity,",
                                    "Reservoir Heat Capacity," + str(input_params.reservoir_heat_capacity)
                                )
                            elif line.startswith("Gradient 1,"):
                                line = line.replace(
                                    "Gradient 1,",
                                    "Gradient 1," + str(input_params.gradient)
                                )
                            lines.append(line)
    ...
    def _perform_run(self, input_file):
        """Perform single GeoPhires run and generate output results"""
        tic = time.time()
        ...
        cpwater = self.heatcapacitywater(Tinj*0.5+(Trock*0.9+Tinj*0.1)*0.5) #J/kg/K (based on TARB in Geophires v1.2)
        ...
        rhowaterprod = self.densitywater(Tprodaverage)  #replace with correlation based on Tprodaverage
        muwaterprod = self.viscositywater(Tprodaverage) #replace with correlation based on Tprodaverage
        ...
        muwaterinj = self.viscositywater(Tinjaverage)*np.linspace(1,1,len(ProducedTemperature))  #replace with correlation based on Tinjaverage
