...
def generate_constraint_sets(model_run):
    ...
    for group_constraint_name, group_constraint in group_constraints.items():
        tech_groups = [
            ...
            if i not in ["techs", "techs_lhs", "techs_rhs", "locs", "locs_lhs", "locs_rhs", "exists"]
        ]
        ...
        techs_lhs = group_constraint.get("techs_lhs", techs)
        techs_rhs = group_constraint.get("techs_rhs", allowed_techs)
        if len(techs_rhs) == 0:
            techs_rhs = allowed_techs
        if len(techs_lhs) == 0:
            techs_lhs = allowed_techs
        techs = list(set(techs_lhs+techs_rhs))

        locs = group_constraint.get("locs", sets["locs"])
        locs_lhs = group_constraint.get("locs_lhs", locs)
        locs_rhs = group_constraint.get("locs_rhs", locs)
        if len(locs_rhs) == 0:
            locs_rhs = sets["locs"]
        if len(locs_lhs) == 0:
            locs_lhs = sets["locs"]
        locs = list(set(locs_lhs+locs_rhs))
    
        # If there are transmission techs, keep only those that link to allowed locations
        techs_lhs = [i for i in techs_lhs if ":" not in techs_lhs or i.split(":")[-1] in locs_lhs]
        techs_rhs = [i for i in techs_rhs if ":" not in techs_rhs or i.split(":")[-1] in locs_rhs]

        trans_techs_lhs = set(techs_lhs).intersection(sets["techs_transmission_names"])
        for i in trans_techs_lhs:
            techs_lhs += [i + ":" + j for j in locs_lhs]
            techs_lhs.remove(i)
        trans_techs_rhs = set(techs_rhs).intersection(sets["techs_transmission_names"])
        for i in trans_techs_rhs:
            techs_rhs += [i + ":" + j for j in locs_rhs]
            techs_rhs.remove(i)

        # If the group constraint defines its own techs, remove those that are not allowed
        techs_lhs = list(set(techs_lhs).intersection(allowed_techs))
        techs_rhs = list(set(techs_rhs).intersection(allowed_techs))

        # All possible loc_techs for this constraint
        loc_techs_all_lhs = list(
            set(concat_iterable([(l, t) for l, t in product(locs_lhs, techs_lhs)], ["::"]))
        )
        loc_techs_all_rhs = list(
            set(concat_iterable([(l, t) for l, t in product(locs_rhs, techs_rhs)], ["::"]))
        )

        # Some loc_techs may not actually exist in the model,
        # so we must filter with actually exising loc_techs
        loc_techs_lhs = [i for i in loc_techs_all_lhs if i in sets.loc_techs]
        loc_techs_rhs = [i for i in loc_techs_all_rhs if i in sets.loc_techs]

        loc_techs = loc_techs_lhs
        default_group_config = checks.DEFAULTS.group_constraints.default_group
        _constraints = {
            k: v
            for k, v in group_constraint.items()
            if k not in ["locs","locs_lhs","locs_rhs", "techs", "techs_lhs","techs_rhs","exists"]
        }
        if any(
            ...
            loc_tech_carrier_dict = _get_carrier_group_constraint_loc_techs(
                loc_techs_lhs, loc_techs_rhs, locs_lhs, locs_rhs, _config, _name, sets, constraint_sets
            )
...
def _get_carrier_group_constraint_loc_techs(
    loc_techs_lhs, loc_techs_rhs, locs_lhs, locs_rhs, config, constraint_name, sets, constraint_sets
):
    ...
    if "net_import" in constraint_name:
        ...
        _loc_tech_carriers = _get_loc_tech_carrier_subset(
            loc_techs_lhs, sets[f"loc_tech_carriers_{flow}"], carrier, locs_lhs
        )
    ...
    if "share" in constraint_name:
        ...
        if "demand" in constraint_name or "import" in constraint_name:
            rhs_loc_tech_carriers = _get_loc_tech_carrier_subset(
                [x for x in loc_techs_rhs if x in sets["loc_techs_demand"]],
                sets["loc_tech_carriers_con"], carrier, locs_rhs
            )          
        elif flow == "con":
            rhs_loc_tech_carriers = _get_loc_tech_carrier_subset(
            ...
            rhs_loc_tech_carriers = _get_loc_tech_carrier_subset(
                [x for x in loc_techs_rhs if x in sets["loc_techs_supply_conversion_all"]],
